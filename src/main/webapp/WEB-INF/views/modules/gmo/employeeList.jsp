...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/gmo/examineemployee/examineentrylist">员工列表</a></li>
	 
	</ul>
	<form:form id="searchForm" modelAttribute="employee" action="${ctx}/gmo/examineemployee/examineentrylist" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>员工状态</th>
				<th>审核状态</th>
				<shiro:hasPermission name="gmo:entryemployee:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="employee">
			<tr>
				<td><a href="${ctx}/gmo/examineemployee/examineentryform?id=${employee.id}">
					${employee.id}
				</a></td>
				<td>
				...
				<td>
					${employee.status == 2?'在职':employee.status == 3?'正在离职':employee.status == 4?'已离职':'正在入职'}
				</td>
				<td style="${employee.entryExamineStatus == 3?'color:red':''}">
					${employee.entryExamineStatus == 2?"审核通过":employee.entryExamineStatus == 3?"审核不通过":"未审核"}
				</td>
				<shiro:hasPermission name="gmo:entryemployee:edit"><td>
    				<a href="${ctx}/gmo/examineemployee/examineentryform?id=${employee.id}">${employee.entryExamineStatus == 1?"审核":"已审核"}</a>
