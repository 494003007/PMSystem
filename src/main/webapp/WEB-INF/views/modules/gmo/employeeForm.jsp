...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/gmo/examineemployee/examineentrylist">员工列表</a></li>
		<li class="active"><a href="${ctx}/gmo/examineemployee/examineentryform?id=${employee.id}">员工<shiro:hasPermission name="per:employee:edit">${not empty employee.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="per:employee:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="employee" action="${ctx}/gmo/examineemployee/examineentrysave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
...
<%-- 		    <td class="labletd"  style="text-align: center;">离职时间  </td>
		   <td>
				...
				<input name="quitDate" type="text" readonly="readonly" maxlength="20" class="input-xlarge Wdate "
		  ...
		  <tr>
		    <td class="labletd"  style="text-align: center;">审核回执</td>
		 	<td colspan="5">
		 	  ...
		 	  <textarea name="entryExamineRemarks"  rows="3" style="width: 99%">${employee.entryExamineRemarks}</textarea>			 
		...
		<shiro:hasPermission name="gmo:entryemployee:edit">
		<c:choose>
			<c:when test="${employee.entryExamineStatus == 3}">
				<input disabled="disabled" id="btnSubmit_no" class="btn btn-danger offset5"  type="button" value="不通过"/>&nbsp;
			</c:when>
			
			<c:when test="${employee.entryExamineStatus == 2}">
...
<script type="text/javascript">
 $("#btnSubmit_ok").click(function(){
	 $("#inputForm").attr("action","${ctx}/gmo/examineemployee/examineentrysave?entryexaminestatus=2");
	 $("#inputForm").submit();
 })
 

$("#btnSubmit_no").click(function(){
	 $("#inputForm").attr("action","${ctx}/gmo/examineemployee/examineentrysave?entryexaminestatus=3");
	 $("#inputForm").submit();
 })
